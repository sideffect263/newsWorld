...
function setupEventListeners() {
  ...
  document.getElementById("saveSourceBtn").addEventListener("click", async () => {
    ...
    if (fetchMethod === "rss") {
      sourceData.feedUrl = document.getElementById("sourceFeedUrl").value;
    ...
    loading.classList.remove("d-none");
    try {
      const url = currentSourceId ? `${API_BASE_URL}/sources/${currentSourceId}` : `${API_BASE_URL}/sources`;
...
function filterSources() {
  ...
  const filteredSources = currentSources.filter((source) => {
    const matchesSearch =
      !searchTerm || source.name.toLowerCase().includes(searchTerm) || source.url.toLowerCase().includes(searchTerm);
    ...
    const matchesStatus =
      !status || (status === "active" && source.isActive) || (status === "inactive" && !source.isActive);

    return matchesSearch && matchesCategory && matchesCountry && matchesLanguage && matchesStatus;
  });
